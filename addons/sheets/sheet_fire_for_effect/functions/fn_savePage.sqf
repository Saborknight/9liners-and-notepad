...
private _key = (profileNamespace getVariable ["nln_fire_for_effect_currentPage", -1]);
...
if (_key <= -1) then
{
	_key = (["fire_for_effect", _values] call nln_fnc_addPage);
	profileNamespace setVariable ["nln_fire_for_effect_currentPage", _key];
	private _pP = (["fire_for_effect", _key] call nln_fnc_getPagePosition);
	hint format["Page %1/%2", (_pP select 0), (_pP select 1)];
}
else
{
	["fire_for_effect", _key, _values] call nln_fnc_setPage;
};

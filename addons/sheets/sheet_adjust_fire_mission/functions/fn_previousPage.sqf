...
if (_lP <= -1) then
{
	_p = (["adjust_fire_mission"] call nln_fnc_getLastPage);
}
else
{
	_p = (["adjust_fire_mission", _lP] call nln_fnc_getPreviousPage);
};
...
if (_p > -1) then
{
	...
	private _pP = (["adjust_fire_mission", _p] call nln_fnc_getPagePosition);

...
if (_key <= -1) then
{
	_key = (["marking_mission", _values] call nln_fnc_addPage);
	profileNamespace setVariable ["nln_marking_mission_currentPage", _key];
	private _pP = (["marking_mission", _key] call nln_fnc_getPagePosition);
	hint format["Page %1/%2", (_pP select 0), (_pP select 1)];
}
else
{
	["marking_mission", _key, _values] call nln_fnc_setPage;
};

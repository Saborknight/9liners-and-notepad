...
if (_lP <= -1) then
{
	_p = (["medevac_nineliner"] call nln_fnc_getLastPage);
}
else
{
	_p = (["medevac_nineliner", _lP] call nln_fnc_getPreviousPage);
};
...
if (_p > -1) then
{
	...
	private _pP = (["medevac_nineliner", _p] call nln_fnc_getPagePosition);

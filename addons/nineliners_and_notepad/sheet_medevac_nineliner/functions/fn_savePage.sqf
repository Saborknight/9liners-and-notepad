...
if (_key <= -1) then
{
	_key = (["medevac_nineliner", _values] call nln_fnc_addPage);
	profileNamespace setVariable ["nln_medevac_nineliner_currentPage", _key];
	private _pP = (["medevac_nineliner", _key] call nln_fnc_getPagePosition);
	hint format["Page %1/%2", (_pP select 0), (_pP select 1)];
}
else
{
	["medevac_nineliner", _key, _values] call nln_fnc_setPage;
};

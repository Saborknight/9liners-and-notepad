...
private _nP = (["adjust_fire_mission", _lP] call nln_fnc_getNextPage);

if (_nP <= -1) then
{
	private _pP = (["adjust_fire_mission", _lP] call nln_fnc_getPagePosition);
...
{
	private _pP = (["adjust_fire_mission", _nP] call nln_fnc_getPagePosition);
